...
public class GameNotificationManager : MonoBehaviour
{
    ...
    void CheckOfflineGrowth()
    {
        ...
        // 비정상적으로 긴 시간 체크 (7일 이상이면 제한)
        if (offlineTime.TotalDays > 7)
        {
            Debug.LogWarning($"오프라인 시간이 너무 깁니다: {offlineTime.TotalDays:F1}일 -> 7일로 제한");
            offlineTime = TimeSpan.FromDays(7);
            lastCloseTime = now.AddDays(-7); // 7일 전으로 조정
        }

        Debug.Log($"오프라인 시간: {offlineTime.TotalHours:F1}시간");
    ...
    {
#if UNITY_ANDROID
        // Android에서는 권한이 자동으로 처리됨 (API Level에 따라)
        Debug.Log("Android 알림 권한 체크 완료");
#elif UNITY_IOS
    ...
    void ScheduleGrowthNotifications()
    {
        ...
        int id1h = AndroidNotificationCenter.SendNotification(notification1h, "cave_growth");
        ...
        int id6h = AndroidNotificationCenter.SendNotification(notification6h, "cave_growth");

        Debug.Log($"Android 알림 스케줄링 완료: 1h={id1h}, 6h={id6h}");
