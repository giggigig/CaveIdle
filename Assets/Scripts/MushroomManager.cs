...
public class MushroomManager : MonoBehaviour
{
    ...
    public MushroomType[] availableMushroomTypes;  // 사용 가능한 버섯 종류들
    ...
    void TrySpawnMushroom()
    {
        ...
        if (spawnPosition != Vector3.zero)
        {
            ...
            Debug.Log($"버섯 생성: 타입({selectedType?.typeName ?? "기본"}), 위치({spawnPosition}), 총 개수: {activeMushrooms.Count}");
        }
    }
